...
export const criarEmpresa = asyncHandler(
  async (req: Request<{}, {}, EmpresaBody>, res: Response) => {
    const empresa = await empresasService.criar(req.body);
    res.status(201).json(empresa);
  },
);

export const editarEmpresa = asyncHandler(
  async (req: Request<{ id: string }, {}, EmpresaBody>, res: Response) => {
    const empresa = await empresasService.editar(req.params.id, req.body);
    res.json(empresa);
  },
);

export const obterConfigFiscal = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
...
export const listarContasBancarias = asyncHandler(
  async (req: Request<{ id: string }>, res: Response) => {
